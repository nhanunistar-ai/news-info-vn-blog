...
files.forEach(file => {
  if (file.endsWith('.md')) {
    ...
    if (tagsMatch) {
      ...
      const seriesTag = tagsSection.match(/- "([a-z-]+)"$/m);
      ...
      const possibleSeriesTags = tagsSection
        ...
        .filter(t => !t.startsWith('chapter-') && !['astro', 'tailwind', '432hz', '440hz', 'music', 'frequency', 'sound-healing', 'tam-ly', 'ai-ky', 'dsm-5-tr', 'van-hoc', 'dien-anh', 'cuoc-doi-ky-la-cua-albert-einstein', 'nhan-vat', 'khoa-hoc', 'cong-nghe', 'lich-su', 'ky-thuat', 'kham-pha'].includes(t));
